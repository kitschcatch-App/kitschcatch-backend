...
public class S3ChatImageStorage implements ChatImageStorage {
    ...
    @Override
    public ChatImageUploadUrl createUploadUrl(
            Long userId,
            Long chatRoomId,
        ...
        String objectKey = objectKey(userId, chatRoomId, originalFileName, normalizedContentType);
    ...
    @Override
    public boolean isOwnedChatImageKey(Long userId, Long chatRoomId, String objectKey) {
        if (!StringUtils.hasText(objectKey) || objectKey.contains("..")) {
            return false;
        }

        String expectedPrefix = properties.chatImagePrefix()
                + "/"
                + chatRoomId
                + "/"
                + userId
    ...
    private String objectKey(
        ...
        return properties.chatImagePrefix()
                + "/"
                + chatRoomId
                + "/"
                + userId
                + "/"
                + UUID.randomUUID()
                + extension(originalFileName, contentType);
    }
